var moveCount=0;//count the moves
var timeInterval;
var seconds=0;
...
function swapSlice(block1, block2) {
    ...
    // Disable transition temporarily for the animated effect
  document.getElementById(block1).style.transition = 'none';
  document.getElementById(block2).style.transition = 'none';

  // Swap classes and HTML content
  ...
  // Enable transition to animate the movement
  setTimeout(function () {
    document.getElementById(block1).style.transition = '';
    document.getElementById(block2).style.transition = '';
  }, 10); // Delay the transition enabling to allow for the DOM update

    moveCount++;//updating the count
    updateMoveCount();
    startStopwatch();
  }
  //shuffle the puzzle to get a solvable one
  function shuffle() {
     
    for (var i = 0; i < 100; i++) {
      var emptySlice = document.getElementsByClassName("slice16")[0];//getting the empty slice
